...
public class CustomerController {
    ...
    public String get(@RequestParam("customerId") int customerId, Model model) {
        Customer customer = customerService.selectByPrimaryKey(customerId);
        //通过分段获取省市区
        String[] split = customer.getCustomerAddress().split(" ");
        //省
        ...
        //详细地址
        String detailAddress = customer.getCustomerAddress().replaceAll(province + " "+ city + " "+ district + " ","");
    ...
    @ResponseBody
    public ResponseCode delete(@RequestParam("customerId") int customerId) {
        int count = customerService.deleteByPrimaryKey(customerId);
        if (count > 0) {
    ...
    public String point(@RequestParam(value = "pn", defaultValue = "1") Integer pn, Model model) {

        PageHelper.startPage(pn, 10);
        List<Customer> list = customerService.selectAllCustomer();
        model.addAttribute("pageInfo", new PageInfo<>(list));
        return "views/customer/points";
    }
}
